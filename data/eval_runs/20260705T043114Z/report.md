# SEAR vs RAG Evaluation Report

- Generated: 2026-07-05T04:31:14.219305+00:00
- Model: models/Mistral-7B-Instruct-v0.3-Q6_K.gguf
- Verifier: overlap (threshold 0.5)
- Query set: data/eval/queryset_twonode.yaml
- Queries: 6
- Wall-clock per query: 51.83s
- Tokens/s (approx): 1.0

## Overall

| Metric | rag | no_continuation |
|---|---|---|
| fabrication_rate | 0.412 | 0.000 |
| grounded_precision | 0.588 | 1.000 |
| attribution_coverage | 0.588 | 1.000 |
| counterfactual_adherence | n/a | n/a |
| failure_precision | n/a | n/a |
| failure_recall | 0.000 | 0.000 |
| cross_node_within_ratio | 0.588 | 1.000 |
| cross_node_violations | 0 | 0 |
| n_claims | 17 | 10 |
| n_queries | 6 | 6 |

## By eval_layer

### out_of_scope

| Metric | rag | no_continuation |
|---|---|---|
| fabrication_rate | 0.000 | 0.000 |
| grounded_precision | 1.000 | 1.000 |
| attribution_coverage | 1.000 | 1.000 |
| counterfactual_adherence | n/a | n/a |
| failure_precision | n/a | n/a |
| failure_recall | 0.000 | 0.000 |
| cross_node_within_ratio | 1.000 | 1.000 |
| cross_node_violations | 0 | 0 |
| n_claims | 1 | 2 |
| n_queries | 1 | 1 |

### realism

| Metric | rag | no_continuation |
|---|---|---|
| fabrication_rate | 0.438 | 0.000 |
| grounded_precision | 0.562 | 1.000 |
| attribution_coverage | 0.562 | 1.000 |
| counterfactual_adherence | n/a | n/a |
| failure_precision | n/a | n/a |
| failure_recall | n/a | n/a |
| cross_node_within_ratio | 0.562 | 1.000 |
| cross_node_violations | 0 | 0 |
| n_claims | 16 | 8 |
| n_queries | 5 | 5 |

## Epistemic quality

| Metric | rag | no_continuation |
|---|---|---|
| query_relevance_rate | 0.833 | 0.833 |
| gold_chunk_coverage | 0.700 | 1.000 |
| supported_irrelevance_rate | 0.000 | 0.200 |
| chunk_quotation_rate | 0.514 | 0.397 |
| divergence_fidelity | 1.000 | 1.000 |

- **rag** failing query relevance: tn_out_of_scope_referendum
- **no_continuation** failing query relevance: tn_out_of_scope_referendum

## Retrieval quality

| Query | gold_recall@k | retrieved | gold |
|---|---|---|---|
| tn_emissions_framing | 1.000 | n1_doc_005:2, n2_doc_001:4, n1_doc_001:7, n1_doc_005:4, n1_doc_007:4 | n1_doc_005:2, n2_doc_001:4 |
| tn_wildfire_framing | 1.000 | n1_doc_008:0, n2_doc_005:1, n1_doc_008:2, n1_doc_008:1 | n1_doc_008:0, n2_doc_005:1 |
| tn_water_framing | 1.000 | n1_doc_009:0, n2_doc_008:2, n2_doc_008:0, n2_doc_008:1, n2_doc_008:4 | n1_doc_009:0, n2_doc_008:2 |
| tn_2023_anomaly | 1.000 | n1_doc_002:1, n1_doc_005:0, n1_doc_007:0, n1_doc_007:1 | n1_doc_002:1 |
| tn_ocean_acidification | 1.000 | n1_doc_004:0, n1_doc_004:4, n1_doc_004:6 | n1_doc_004:0 |
| tn_out_of_scope_referendum | n/a | election_centralwire:0, election_metrodaily:0, port_authority_brief:0, school_district_report:0 | — |

- **rag** mean gold recall@k: 1.000
- **no_continuation** mean gold recall@k: 1.000
