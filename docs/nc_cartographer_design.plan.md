...
**Escalation judge calibration, honest harness** (2026-07-13, `scripts/cartographer_eval_escalation.py`, `gin/cartographer/escalation_eval.py`): the §6 "Mistral-7B collapses to a constant answer" finding was re-examined and is **substantially a harness artifact, not only a model limit**. Three controlled runs against the same pairs: one-word/8-token budget (`20260713T071808Z`) → constant DIVERGENT (recall 1.0 trivially, class_c 0.0); reasoning budget but with a register exemplar ("official/technical vs community/justice") inside the DIVERGENT definition (`20260713T083941Z`) → still constant DIVERGENT, same numbers — the exemplar alone reproduces the collapse; reasoning budget, register-neutral wording (`20260713T092010Z`) → labels genuinely mix (recall 0.5, class_c 0.67, unrelated 0.25) but **7 of 14 pairs flip label under argument-order swap**, and near-identical caveat pairs (labor/wage/inflation/export) draw four different label combinations. Conclusion: with a fair prompt, Mistral-7B's verdicts on issue-frame divergence are order-unstable noise, not a usable signal at any prompt setting — a capacity finding the one-word harness could not have distinguished from a prompt-bias artifact. `FRAME_JUDGE_PROMPT` wording is therefore calibration-sensitive; any edit needs a rerun. Calibration set expanded beyond the 4 issue_frame + 2 CLASS_C pairs with `ESCALATION_CLASS_C_EXTRA` / `ESCALATION_UNRELATED_CONTROLS` (`gin/cartographer/gold_edges.py`, kept separate from `CLASS_C_CONTROLS` so the closed scan-eval metric denominator is untouched) plus the free 33-pair labeled-set breadth block. Next candidate: 14B+ local instruct (fits a 12 GB GPU) or an API judge, calibrated the same way before trusting it.
