...
1. **NLI re-run on expanded 20-query set** — **Done** (`20260712T035228Z`, Mistral Q6_K, GPU `n_gpu_layers=-1`, WSL Ubuntu venv, model `models/Mistral-7B-Instruct-v0.3-Q6_K.gguf`). NC `fabrication_rate` **0.056** under NLI (1/18 graded claims UNSUPPORTED on `unemployment_rate` counterfactual — verbatim extract, NLI entailment miss; realism layer **0.000**). RAG `fabrication_rate` **0.714** as expected on paraphrase. `corpus_fingerprint` recorded in `meta.json`. Native Windows Python 3.14 still crashes at model load (`0xc000001d`); use WSL venv for live generation.

2. **RAG under NLI** — High fabrication on paraphrase; overlap is fairer for RAG comparison.

3. **Representative GPU hardware** — **Measured and root-caused** (see prior text). **Promotion rule updated** (ENG 00): fabrication 0.000 on both backends required; ≤1/20 boundary-decision flips tolerated.

4. **Verifier min-length floor** — Short fragment claims (e.g. truncated numerics) can score overlap 1.0; convergent sentence-end close reduces but does not eliminate; harness tweak still optional.

5. **Retrieval non-determinism across ingestion runs** — **Mitigated** (2026-07-12): `rerank_hits_by_query_score` tie-breaks on `chunk_id`; eval `meta.json` records `corpus_fingerprint` for apples-to-apples comparison. Re-ingest comparison policy: match fingerprints only.

6. **Cartographer batch wiring** — **Done** (2026-07-12): `scripts/cartographer_scan.py`, `gin/bookkeeper/persist.py`, provenance columns on `edges`, E2E `tests/test_cartographer_scan_e2e.py`. **Production validation** (2026-07-12): scan-first ingest (`--no-edges`), scan-vs-gold harness (`scripts/cartographer_eval_scan.py`), admitted-anchor read path, and divergence eval battery with `edge_source: cartographer_scan` in `meta.json`. Baseline scan recall on 8 gold contradicts = 0.50 with 135 false positives (`20260712T053645Z`, `--cross-outlet-only`). **Scan precision pass** (`20260712T074956Z`): IDF+embedding hybrid prune, doc-pair dedup, Bookkeeper relation re-check (`gin/bookkeeper/relation_verify.py`) — false positives **120**, recall **0.50**, `class_c_discrimination` **1.0**; housing alderflats gold pair recovered. Eval: framing2 matches baseline (`20260712T060050Z`, fidelity 1.0); twonode/multipara/housing partial or fail — threshold calibration (§6b) and detector mis-types still open.
