...
A final-review finding is recorded rather than quietly fixed: the chunk-level leakage guard was **insufficient**, because the fixture corpus aliases bar chunks under different ids with byte-identical text (`inst_em:0` *is* `n1_doc_005:2`). Three of the bar's four issue_frame pairs were in training verbatim. `build_dataset` now guards on text as well (`bar_text_alias`), which cut the training set from 80 rows to 49. `readiness.py` counted at the chunk-id level and was subject to the same aliasing, so the gauge overstated readiness; it now excludes on text via the shared `gin/curator/text_index.py` and agrees with what the detector can actually train on. **Corrected, it reads issue_frame 24 / agree 9 / unrelated 20 against a 20/class target — still NOT ready.** B was in fact built on a corpus that had not met its own gate. The UNRELATED shortfall has since been closed (14 labels attributed `curator: "claude"`, lifting the training set to 63 rows and LOO to 0.799); AGREE was then closed by the human curator, so **the gate is green at 102 rows** (DIVERGENT 24 / AGREE 20 / RELATED_UNTYPED 38 / UNRELATED 20). At that size three of four bar metrics are green — only `issue_frame_recall` still fails, at 0.00 — and LOO settles to 0.705, lower than the 63-row 0.799 because 44 added hard negatives exposed that **AGREE and RELATED_UNTYPED are symmetrically confused** (12 vs 13 misclassifications, near chance). The sharpened finding: the frozen geometry separates *topical* distinctions and fails on *epistemic* ones — both `issue_frame` and the corroborates/related-untyped boundary are epistemic and neither is learnable here, and closing the label gate did not change that, which makes the missing signal representational rather than a matter of quantity. The added negatives are unambiguous cross-domain pairs, so UNRELATED recall 1.000 partly reflects that selection — DIVERGENT (0.917), RELATED_UNTYPED (0.500) and every bar metric are unchanged. Spec + full results: **[docs/superpowers/specs/2026-07-24-bi-encoder-frame-detector-design.md](docs/superpowers/specs/2026-07-24-bi-encoder-frame-detector-design.md)**.

**Calibration tier (sub-project C, measured 2026-07-25).** The cheap pipeline's thresholds were grid-searched over 39 sample tuples hand-transcribed in July 2026 — stale, and overlapping the pipeline's own eval set. Calibration samples are now a **generated file** (`data/calibration/samples.json`, produced by `scripts/regen_calibration_samples.py`, read behind a manifest gate on model ids and `same_story` parameters), the 40 held-out eval pairs live in a separate array `calibrate()` structurally cannot read, and `gin/cartographer/eval_pairs.py` **pins the escalation bar's 14 pairs by chunk id** so a pre-registered eval can no longer move silently.

**The recalibration itself was measured and rejected**, and `data/cartographer_thresholds.json` deliberately retains its earlier values. On 131 curated samples, LOO fell 0.897 → 0.534 and contradicts precision *and* recall were both 0.0 — structural, not a bad threshold choice. `classify_relation` gates both contradicts channels on `same_story`: `False` blocks CONTRADICTS (all 22 contradicts samples) while `True` forces it (11 rows, none gold contradicts), leaving 33 of 131 rows unfixable at any threshold. The decisive check: on the 40 frozen eval pairs the retained thresholds score **0.700** against the recalibrated **0.550**.

The finding is that **the curated corpus and the cheap pipeline target different phenomena** — the pipeline detects same-story propositional conflict, while curation has grown toward cross-document framing divergence, and only 11 of 131 samples are same-story at all. That mirrors sub-project B's result from another angle: curation and the automated machinery have drifted apart in subject matter, and that drift is the thing to fix rather than any threshold. Known limitation: `calibrate()` is O(n⁴) and `leave_one_out()` O(n⁵) — this run took ~2.25 h at n=131 and the method is impractical past ~150 samples without vectorizing `_score()`. Spec + full results: **[docs/superpowers/specs/2026-07-25-cheap-pipeline-recalibration-design.md](docs/superpowers/specs/2026-07-25-cheap-pipeline-recalibration-design.md)**.

**Same-story corpus tier (sub-project D, measured 2026-07-25).** Sub-project C's finding — the cheap pipeline detects same-story conflict, not framing divergence — motivated re-aiming curation at the phenomenon the machinery already handles: `combined.py`'s unconditional `if same_story: return CONTRADICTS` branch, which a corpus of pure conflicts could never falsify. `data/curator/node5_events.yaml` (authored, reviewed) builds through `scripts/build_node5.py` into `corpus_node5.json`: **12 events, 38 outlet reports, 42 authored pairs** (21 conflicts, 21 same-story negatives — 11 corroboration, 4 update, 6 compatible_partial), surfaced to the curator by a new `SameStoryCandidateSource` (`gin/curator/same_story.py`) that ranks NLI-contradiction-descending but never filters on it, so the negatives — the reason the corpus exists — are never dropped. The surfacing gate (`gin/curator/node5_verify.py`, `scripts/verify_node5_surfacing.py`) initially found only **6 of 42** pairs reachable: `make_same_story`'s entity-anchor requires a shared rare token, and rarity is document frequency computed over whatever corpus the predicate is built from — over node5's 38 chunks alone the rare-token ceiling is 2, but each event's shared lede repeats identically across that event's 3–4 reports (df 3–4), so a lede could never be rare enough to anchor its own event, regardless of wording; only pairs that happened to also repeat an unvaried numeric detail passed by chance. The fix is in the predicate's *inputs*, not the corpus or `make_same_story` itself: the gate and the launcher (`scripts/verify_node5_surfacing.py`, `scripts/curator_serve.py --source same-story`) build `same_story` over node5's texts **plus** `default_text_index()` (274 docs, ceiling 9) — the realistic corpus scale the pipeline runs over in production — so a df-3–4 lede is comfortably rare against the wider index. Re-run: **`authored 42 | surfaced 42`, PASS**. The curator smoke (`GET /curator/`, `GET /curator/readiness`) returned 200 with `new_story: 13` against a `story: 20` target. **No detector metric was measured** — this ships a corpus and the means to label it; whether `combined.py`'s same-story branch survives labeling is the follow-on's question. Spec: **[docs/superpowers/specs/2026-07-25-same-story-corpus-node5-design.md](docs/superpowers/specs/2026-07-25-same-story-corpus-node5-design.md)**.
